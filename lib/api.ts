...
class ApiClient {
  ...
  constructor() {
    if (typeof window !== 'undefined') {
      ...
      console.log('[ApiClient] Initialized with stored token:', !!storedToken)
    } else {
      this.token = null
    }
  }
  ...
  private getBaseURL(): string {
    ...
    // For localhost development, always use Next.js proxy to ensure proper routing
    if (host === 'localhost' || host === '127.0.0.1' || host.includes('localhost')) {
      // Always use relative path for API calls to use Next.js proxy
      const url = '/api'
  ...
  private async request<T>(
    ...
    // Always check for the latest token from localStorage before each request
    if (typeof window !== 'undefined') {
      const currentToken = localStorage.getItem('auth_token')
      if (currentToken) {
        this.token = currentToken
      } else if (!currentToken && this.token) {
        // Token was removed from localStorage, clear our copy
        this.token = null
      }
    }
    ...
    try {
      ...
      if (error instanceof TypeError && error.message === 'Failed to fetch' && retryCount < 3) {
        console.warn(`[API Client] Network error, retrying... (${retryCount + 1}/3)`, url)
