...
describe('DataTable Component', () => {
  ...
  describe('View Toggle Functionality', () => {
    test('shows view toggle buttons when enabled', () => {
      ...
      const viewToggleButtons = buttons.filter(button => 
        button.querySelector('svg') // Icon buttons for view toggle
      )
    ...
    test('switches between table and card view', async () => {
      ...
      const cardViewButton = screen.getByRole('button', { name: /card view/i }) ||
                             screen.getAllByRole('button').find(btn => 
                               btn.querySelector('svg')?.getAttribute('class')?.includes('lucide-layout-grid')
                             )
  ...
  describe('Persistent State Management', () => {
    ...
    test('loads table state from localStorage', () => {
      ...
      expect(localStorageMock.getItem).toHaveBeenCalledWith('datatable-referee-state')
    })
