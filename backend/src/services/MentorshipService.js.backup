 ...
 * lifecycle management, and authorization controls.
 * 
 * Key Features:
...
class MentorshipService extends BaseService {
   ...
   * @param {Object} mentorshipData - Mentorship data
   * @param {string} mentorshipData.mentor_id - ID of the mentor user
   * @param {string} mentorshipData.mentee_id - ID of the mentee user
   * @param {string} mentorshipData.start_date - Start date of the mentorship
   * @param {string} [mentorshipData.notes] - Optional initial notes
   * @param {Object} options - Creation options
   * @returns {Object} Created mentorship record
   * @throws {Error} If mentor eligibility check fails or relationship already exists
   */
  async createMentorship(mentorshipData, options = {}) {
    try {
      ...
      const mentorship = await this.create(mentorshipRecord, options);
   ...
   * @param {string} [options.status] - Filter by mentorship status
   * @param {boolean} [options.includeDetails] - Include mentee details
   * @returns {Array} Array of mentorship records
   */
  async getMentorshipsByMentor(mentorId, options = {}) {
    try {
      ...
      const mentorships = await query.orderBy('mentorships.start_date', 'desc');
   ...
   * @param {Object} options - Query options
   * @param {string} [options.status] - Filter by mentorship status
   * @param {boolean} [options.includeDetails] - Include mentor details
   * @returns {Array} Array of mentorship records
   */
  async getMentorshipsByMentee(menteeId, options = {}) {
    try {
      const { status, includeDetails = true } = options;
   ...
   * @param {Object} options - Query options
   * @returns {Object} Mentorship record with details
   * @throws {Error} If user doesn't have access to the mentorship
   */
  async getMentorshipWithDetails(mentorshipId, requestingUserId, options = {}) {
    try {
   ...
   * @param {string} mentorshipId - Mentorship ID
   * @param {string} newStatus - New status (active, paused, completed, terminated)
   * @param {string} mentorId - Mentor ID (for authorization)
   * @param {Object} options - Update options
   * @returns {Object} Updated mentorship record
   * @throws {Error} If status transition is invalid or user lacks permission
   */
  async updateMentorshipStatus(mentorshipId, newStatus, mentorId, options = {}) {
    try {
      // Validate status
      ...
      const updatedMentorship = await this.update(mentorshipId, updateData, options);
   ...
   * @returns {boolean} True if user can be a mentor
   */
  async canUserBeMentor(userId) {
    try {
   ...
   */
  async getMentorshipStats(mentorId) {
    try {
   ...
   */
  async validateMentorEligibility(userId) {
    try {
   ...
   */
  async checkExistingRelationship(mentorId, menteeId) {
    try {
   ...
   */
  async validateUsers(mentorId, menteeId) {
    try {
      // Check if both users exist
      const [mentor, mentee] = await Promise.all([
        this.db('users').where('id', mentorId).first(),
        this.db('users').where('id', menteeId).first()
      ]);
   ...
   */
  async findAvailableMentors(menteeId, options = {}) {
    try {
   ...
   * @param {Object} mentorship - Created mentorship
   * @param {Object} options - Creation options
   */
  async afterCreate(mentorship, options) {
   ...
   * @param {Object} mentorship - Updated mentorship
   * @param {Object} previousMentorship - Previous mentorship state
   * @param {Object} options - Update options
   */
  async afterUpdate(mentorship, previousMentorship, options) {
