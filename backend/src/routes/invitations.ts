...
router.post('/', authenticateToken, requireCerbosPermission({
  ...
  try {
    ...
    const { email, first_name, last_name, role } = value as InviteRequest;
...
router.get('/', authenticateToken, requireCerbosPermission({
  ...
  try {
    ...
    const invitations = await query.limit(limitNum).offset(offset);
...
router.post('/:token/complete', async (req: Request, res: Response) => {
  try {
    ...
    const { password, phone, location, postal_code, level, max_distance } = value as CompleteInvitationRequest;
