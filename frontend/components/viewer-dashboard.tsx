...
import { ViewerDashboardOverview } from '@/components/viewer-dashboard-overview'
...
export function ViewerDashboard() {
  ...
  const checkViewPermission = (view: string): boolean => {
    // Viewer role-specific permissions (read-only)
    if (user?.role === 'viewer') {
      const allowedViews = ['dashboard', 'games', 'calendar', 'resources', 'profile']
  ...
  const renderContent = () => {
    ...
    switch (activeView) {
    case 'dashboard':
      return <ViewerDashboardOverview />
    case 'games':
      // Read-only view of games
      return <GamesManagementPage initialDateFilter={gameManagementDateFilter} readOnly={true} />
    case 'calendar':
      return <CalendarView 
      ...
      />
    case 'resources':
      return <ResourceCentreNew readOnly={true} />
    case 'profile':
