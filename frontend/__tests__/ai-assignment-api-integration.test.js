...
describe('AI Assignment API Integration', () => {
  ...
  describe('AI Assignment Rule CRUD Operations', () => {
    test('creates AI assignment rule with correct parameters', async () => {
      ...
      expect(mockApi.createAIAssignmentRule).toHaveBeenCalledWith(expectedRuleData)
    ...
    test('creates LLM-based AI assignment rule', async () => {
      const llmRuleData = {
        ...
        enabled: true,
        schedule: { type: 'recurring', frequency: 'weekly', dayOfWeek: 'monday', time: '09:00' },
        criteria: {
          gameTypes: [],
          ageGroups: [],
          maxDaysAhead: 30,
    ...
    test('handles AI rule creation errors', async () => {
      ...
      mockApi.createAIAssignmentRule.mockRejectedValue(new Error('Validation failed'))

      await expect(mockApi.createAIAssignmentRule(invalidRuleData))
    ...
    test('updates AI assignment rule', async () => {
      ...
      const result = await mockApi.updateAIAssignmentRule('rule-123', updateData)

      expect(mockApi.updateAIAssignmentRule).toHaveBeenCalledWith('rule-123', updateData)
      expect(result.success).toBe(true)
  ...
  describe('AI Assignment Rule Execution', () => {
    test('runs AI assignment rule with dry run', async () => {
      ...
      const expectedResponse = {
        ...
        data: {
          ...
          algorithmicScores: {
            weights: { distance: 0.4, skill: 0.3, experience: 0.2, partner: 0.1 },
      ...
      expect(mockApi.runAIAssignmentRule).toHaveBeenCalledWith('rule-123', runParams)
      expect(result.success).toBe(true)
    ...
    test('handles rule execution with conflicts', async () => {
      ...
      const responseWithConflicts = {
        ...
        data: {
          ...
          assignments: [
            {
              gameId: 'game-1',
              assignedReferees: [{ refereeId: 'ref-1', refereeName: 'John Smith' }],
      ...
      expect(result.data.assignments[0].conflicts).toContain('Insufficient referees available')
    })

    test('handles rule execution errors', async () => {
      mockApi.runAIAssignmentRule.mockRejectedValue(new Error('Rule execution failed'))

      await expect(mockApi.runAIAssignmentRule('invalid-rule', {}))
  ...
  describe('AI Assignment Rule Run History', () => {
    ...
    test('fetches rule runs with filters', async () => {
      ...
      expect(mockApi.getAIAssignmentRuleRuns).toHaveBeenCalledWith('rule-123', filters)
    })
  })
  ...
  describe('Partner Preferences Management', () => {
    test('adds partner preference for algorithmic rules', async () => {
      ...
      const result = await mockApi.addPartnerPreference('rule-123', preferenceData)

      expect(mockApi.addPartnerPreference).toHaveBeenCalledWith('rule-123', preferenceData)
      expect(result.success).toBe(true)
    ...
    test('adds avoid partner preference', async () => {
      ...
      const result = await mockApi.addPartnerPreference('rule-123', avoidPreference)
    ...
    test('deletes partner preference', async () => {
      ...
      const result = await mockApi.deletePartnerPreference('rule-123', 'pref-123')

      expect(mockApi.deletePartnerPreference).toHaveBeenCalledWith('rule-123', 'pref-123')
      expect(result.success).toBe(true)
    })
    ...
    test('handles partner preference validation errors', async () => {
      ...
      mockApi.addPartnerPreference.mockRejectedValue(new Error('Invalid preference data'))

      await expect(mockApi.addPartnerPreference('rule-123', invalidPreference))
  ...
  describe('Data Transformation for Enterprise Component', () => {
    test('transforms backend games data for frontend', async () => {
      ...
      const result = await mockApi.getGames({ status: 'unassigned', limit: 100 })
      ...
      expect(result.data[0].payRate).toBe(85.00) // Transformed from pay_rate
    })
    ...
    test('transforms backend referees data for frontend', async () => {
      ...
      expect(result.data.referees[0].wagePerGame).toBe(75.00) // Transformed from wage_per_game
  ...
  describe('API Error Handling', () => {
    test('handles network errors gracefully', async () => {
      mockApi.createAIAssignmentRule.mockRejectedValue(new Error('Network error'))

      await expect(mockApi.createAIAssignmentRule({}))
    ...
    test('handles HTTP error responses', async () => {
      ...
      await expect(mockApi.runAIAssignmentRule('nonexistent-rule', {}))
