  ...
  describe('Complete Content Creation Workflow', () => {
    test('should create content with attachments and search indexing', async () => {
      ...
      const contentResponse = await fetch(
        'http://localhost:3001/api/content/items',
        {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            ...
            content:
              '<h1>Training Overview</h1><p>This comprehensive guide covers <strong>advanced techniques</strong> for basketball referees.</p>',
      ...
      const getResponse = await fetch(
        `http://localhost:3001/api/content/items/${content.id}`
      );
      ...
      const searchResponse = await fetch(
        'http://localhost:3001/api/content/search?q=referee training'
      );
      ...
      const updateResponse = await fetch(
        `http://localhost:3001/api/content/items/${content.id}`,
        {
          ...
          body: JSON.stringify({
            ...
            content:
              '<h1>Updated Training Overview</h1><p>This guide has been <strong>updated</strong> with new techniques.</p>',
          }),
        }
      );
      ...
      const versionsResponse = await fetch(
        `http://localhost:3001/api/content/items/${content.id}/versions`,
        {
    ...
    test('should handle file deduplication across content items', async () => {
      ...
      const content1Response = await fetch(
        'http://localhost:3001/api/content/items',
        {
          method: 'POST',
          headers: {
      ...
      const content2Response = await fetch(
        'http://localhost:3001/api/content/items',
        {
          method: 'POST',
          headers: {
      ...
      formData1.append(
        'file',
        new Blob([sameImageBuffer], { type: 'image/jpeg' }),
        'shared-image.jpg'
      );
      ...
      const fullContent1 = await fetch(
        `http://localhost:3001/api/content/items/${content1.id}`
      ).then((r) => r.json());
      const fullContent2 = await fetch(
        `http://localhost:3001/api/content/items/${content2.id}`
      ).then((r) => r.json());
    ...
    test('should maintain search index consistency', async () => {
      ...
      const content1 = await fetch('http://localhost:3001/api/content/items', {
        ...
        body: JSON.stringify({
          ...
          content:
            '<h1>Basic Skills</h1><p>Learn fundamental <strong>basketball</strong> officiating skills.</p>',
          category_id: 1,
      ...
      const content2 = await fetch('http://localhost:3001/api/content/items', {
        ...
        body: JSON.stringify({
          ...
          content:
            '<h1>Advanced Methods</h1><p>Advanced <strong>basketball</strong> officiating techniques.</p>',
          category_id: 1,
      ...
      const basketballSearch = await fetch(
        'http://localhost:3001/api/content/search?q=basketball'
      ).then((r) => r.json());

      expect(basketballSearch.results).toHaveLength(2);

      // Search for 'fundamentals' should return only first
      const fundamentalsSearch = await fetch(
        'http://localhost:3001/api/content/search?q=fundamentals'
      ).then((r) => r.json());

      expect(fundamentalsSearch.results).toHaveLength(1);
      expect(fundamentalsSearch.results[0].title).toBe(
        'Basketball Training Fundamentals'
      );
      ...
      await fetch(`http://localhost:3001/api/content/items/${content1.id}`, {
        ...
        body: JSON.stringify({
          ...
          content:
            '<h1>Basic Skills</h1><p>Learn basic <strong>basketball</strong> officiating skills.</p>',
          search_keywords: ['basketball', 'training', 'basics'],
        }),
      });

      // Search for 'fundamentals' should now return empty
      const updatedFundamentalsSearch = await fetch(
        'http://localhost:3001/api/content/search?q=fundamentals'
      ).then((r) => r.json());
      ...
      const basicsSearch = await fetch(
        'http://localhost:3001/api/content/search?q=basics'
      ).then((r) => r.json());
    ...
    test('should handle content deletion and cleanup', async () => {
      ...
      formData.append(
        'file',
        new Blob([Buffer.from('test-file')], { type: 'application/pdf' }),
        'test.pdf'
      );
      formData.append('content_item_id', content.id.toString());
      ...
      const searchBefore = await fetch(
        `http://localhost:3001/api/content/search?q=delete`
      ).then((r) => r.json());
      ...
      const attachmentResponse = await fetch(
        `http://localhost:3001/api/content/media/${attachment.id}`
      );
