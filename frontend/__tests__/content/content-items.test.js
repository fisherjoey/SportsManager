...
describe('Content Items Database Operations', () => {
  ...
  beforeEach(() => {
    ...
    contentDb.searchContentByKeywords.mockImplementation(async (keywords) => {
      ...
      if (keywords.includes('basketball')) {
        const item1 = {
          id: 1,
          title: 'Basketball Training Manual',
          content: 'Basketball referee guide',
        };
        results.push(item1);
        added.add(1);
        ...
        const item3 = {
          ...
          content: 'Basketball game day protocols',
        };
        results.push(item3);
        added.add(3);
      }
  ...
  describe('Content Creation', () => {
    test('should create content item with HTML content', async () => {
      const contentData = {
        ...
        content:
          '<h1>Welcome</h1><p>This is a <strong>test</strong> document with <em>rich formatting</em>.</p>',
        category_id: 1,
        type: 'document',
        status: 'draft',
        visibility: 'public',
    ...
    test('should extract plain text from HTML for search', async () => {
      const htmlContent =
        '<h2>Referee Guidelines</h2><p>All referees must <strong>arrive 30 minutes early</strong> and bring <em>proper equipment</em>.</p><ul><li>Whistle</li><li>Cards</li><li>Watch</li></ul>';
      ...
      contentDb.extractPlainText.mockReturnValue(
        'Referee Guidelines All referees must arrive 30 minutes early and bring proper equipment. Whistle Cards Watch'
      );
  ...
  describe('Content Updates', () => {
    ...
    test('should update slug when title changes', async () => {
      const content = contentDb.createContentItem.mockResolvedValue({
        id: 1,
        slug: 'meeting-notes',
      });
      await contentDb.createContentItem({
  ...
  describe('Content Search', () => {
    beforeEach(async () => {
      ...
      await contentDb.createContentItem({
        title: 'Referee Training Manual',
        content:
          '<h1>Advanced Referee Techniques</h1><p>This manual covers advanced officiating techniques for basketball referees.</p>',
        category_id: 1,
        status: 'published',
      ...
      await contentDb.createContentItem({
        title: 'Game Day Procedures',
        content:
          '<p>Pre-game procedures for all basketball officials and referees.</p>',
        category_id: 1,
        status: 'published',
    ...
    test('should perform full-text search', async () => {
      ...
      expect(results[0].rank).toBeGreaterThanOrEqual(
        results[results.length - 1].rank
      );
    });

    test('should search by keywords', async () => {
      const results = await contentDb.searchContentByKeywords([
        'basketball',
        'referee',
      ]);

      expect(results.length).toBe(2);
      expect(results.some((r) => r.title.includes('Training Manual'))).toBe(
        true
      );
