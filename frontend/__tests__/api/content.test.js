...
describe('/api/content/items', () => {
  ...
  describe('POST /api/content/items', () => {
    test('should create content item successfully', async () => {
      ...
      const { req, res } = createMocks({
        ...
        body: {
          ...
          content:
            '<h1>Training Overview</h1><p>This document covers <strong>essential training</strong> for all referees.</p>',
          category_id: 1,
          type: 'document',
          visibility: 'public',
        },
    ...
    test('should sanitize HTML content', async () => {
      ...
      const { req } = createMocks({
        ...
        body: {
          ...
          content:
            '<h1>Title</h1><script>alert("xss")</script><p>Safe content</p><img src="x" onerror="alert(1)">',
          category_id: 1,
        },
        headers: {
          authorization: 'Bearer valid-jwt-token',
    ...
    test('should handle large content efficiently', async () => {
      ...
      const largeContent =
        ...
        '<p>'.repeat(1000) +
        'This is a large document with lots of content. '.repeat(100) +
        '</p>'.repeat(1000);
  ...
  describe('PUT /api/content/items/[id]', () => {
    test('should update content item', async () => {
      ...
      const { req } = createMocks({
        ...
        body: {
          ...
          content:
            '<h1>Updated Content</h1><p>This content has been <em>updated</em>.</p>',
          status: 'published',
        },
        headers: {
          authorization: 'Bearer valid-jwt-token',
    ...
    test('should create version on update', async () => {
      ...
      const {
        GET,
      } = require('../../app/api/content/items/[id]/versions/route');
      const versionsResponse = await GET(new Request('http://localhost'), {
        params: Promise.resolve({ id: '1' }),
      });
