...
describe('/api/content/upload', () => {
  ...
  beforeAll(() => {
    ...
    fs.writeFileSync(path.join(testFilesDir, 'test-document.pdf'), Buffer.from('%PDF-1.4\n%test pdf content'));
  });
  ...
  describe('POST /api/content/upload', () => {
    test('should upload image and return URL', async () => {
      ...
      const imageBuffer = fs.readFileSync(path.join(testFilesDir, 'test-image.png'));
      ...
      expect(data).toMatchObject({
        ...
        file_url: expect.stringMatching(/^\/uploads\/images\/\d{4}\/\d{2}\/[a-f0-9]{64}\.png$/),
    ...
    test('should detect and prevent duplicate uploads', async () => {
      ...
      formData1.append('file', new Blob([imageBuffer], { type: 'image/png' }), 'duplicate1.png');
      
      const formData2 = new FormData();
      formData2.append('file', new Blob([imageBuffer], { type: 'image/png' }), 'duplicate2.png');
    ...
    test('should validate file types', async () => {
      ...
      const formData = new FormData();
      formData.append('file', new Blob([executableContent], { type: 'application/x-executable' }), 'malware.exe');
      ...
      expect(data.allowed_types).toEqual(
        expect.arrayContaining(['image/jpeg', 'image/png', 'image/gif', 'application/pdf', 'video/mp4'])
      );
    });
    ...
    test('should enforce file size limits', async () => {
      ...
      const formData = new FormData();
      formData.append('file', new Blob([largeBuffer], { type: 'image/jpeg' }), 'large-image.jpg');
    ...
    test('should require authentication', async () => {
      ...
      const formData = new FormData();
      formData.append('file', new Blob([imageBuffer], { type: 'image/png' }), 'unauthorized.png');
    ...
    test('should handle TinyMCE image uploads', async () => {
      ...
      const formData = new FormData();
      formData.append('file', new Blob([imageBuffer], { type: 'image/png' }), 'tinymce-image.png');
  ...
  describe('DELETE /api/content/media/[id]', () => {
    ...
    test('should handle files referenced by multiple content items', async () => {
      ...
      const data = await response.json();
      expect(data.error).toContain('File is referenced by multiple content items');
