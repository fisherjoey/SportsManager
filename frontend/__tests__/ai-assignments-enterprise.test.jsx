...
beforeEach(() => {
  ...
  mockApi.runAIAssignmentRule.mockResolvedValue({
    success: true,
    data: { assignments: [], conflictsFound: 0 },
  });
...
describe('AIAssignmentsEnterprise Component', () => {
    ...
    test('handles data loading errors gracefully', async () => {
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith(
          'Failed to load real data, using mock data:',
          expect.any(Error)
        );
      });
  ...
  describe('Statistics and Metrics Display', () => {
    test('calculates and displays unassigned games count', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Games needing assignment')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Optimization Settings Tab', () => {
    test('renders optimization parameters with default values', async () => {
      ...
      expect(
        screen.getByText('Max Games per Referee per Week')
      ).toBeInTheDocument();
      expect(screen.getByText('Max Travel Distance (km)')).toBeInTheDocument();
      expect(
        screen.getByText('Preferred Division Weight (%)')
      ).toBeInTheDocument();
      expect(screen.getByText('Experience Weight (%)')).toBeInTheDocument();
      expect(
        screen.getByText('Workload Balance Weight (%)')
      ).toBeInTheDocument();
      expect(
        screen.getByText('Minimum Rest Between Games (minutes)')
      ).toBeInTheDocument();
    });
    ...
    test('displays optimization tip warning', async () => {
      ...
      expect(
        screen.getByText(
          'Higher weights prioritize that factor more heavily. Balance all weights to achieve optimal results.'
        )
      ).toBeInTheDocument();
    });
  ...
  describe('Bulk Assignment Workflow', () => {
    test('renders bulk assignment interface', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('All Unassigned Games (2 games)')
        ).toBeInTheDocument();
        expect(
          screen.getByText('Selected Games (0 games)')
        ).toBeInTheDocument();
      });
    });
    ...
    test('shows bulk assignment mode information', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(
            /Will process all 2 unassigned games using advanced optimization algorithms/
          )
        ).toBeInTheDocument();
        expect(
          screen.getByText(/Estimated processing time: 1 seconds/)
        ).toBeInTheDocument();
      });
    });
    ...
    test('executes bulk assignment with backend integration', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Processing assignments...')
        ).toBeInTheDocument();
  ...
  describe('Game Selection Interface', () => {
    test('renders game selection list', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Select specific games for assignment (optional)')
        ).toBeInTheDocument();
    ...
    test('allows individual game selection', async () => {
      ...
      await waitFor(() => {
        ...
        const selectedGamesScope = screen.getByLabelText(
          'Selected Games (1 games)'
        );
  ...
  describe('Assignment Results Tab', () => {
    ...
    test('displays assignment results after generation', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText(
            'Successfully assigned 1 games with optimization metrics'
          )
        ).toBeInTheDocument();
    ...
    test('displays individual assignment details', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Optimal assignment based on AI algorithm')
        ).toBeInTheDocument();
      });
    });
  ...
  describe('Error Handling and Edge Cases', () => {
    test('handles empty games list', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('All Unassigned Games (0 games)')
        ).toBeInTheDocument();
      });
    });
