...
describe('BudgetTracker Component', () => {
  ...
  describe('Initial Rendering and Data Loading', () => {
    ...
    it('displays error when initial data loading fails', async () => {
      jest
        .mocked(apiClient.getBudgetPeriods)
  ...
  describe('Budget Creation', () => {
    it('opens create budget modal when New Budget button is clicked', async () => {
      ...
      const newBudgetButton = screen.getByRole('button', {
        name: /new budget/i,
      });
    ...
    it('creates budget successfully with valid data', async () => {
      ...
      const periodSelect = screen.getByRole('button', {
        name: /select period/i,
      });
    ...
    it('handles budget creation failure with specific error messages', async () => {
      jest
        .mocked(apiClient.createBudget)
        .mockRejectedValue(new Error('duplicate budget name'));
      ...
      await waitFor(() => {
        expect(toast).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            description:
              'A budget with this name already exists in the selected period.',
          })
        );
      });
    });
  ...
  describe('Budget Editing', () => {
    ...
    it('handles update failure with specific error message', async () => {
      jest
        .mocked(apiClient.updateBudget)
        .mockRejectedValue(new Error('404 budget not found'));
      ...
      await waitFor(() => {
        expect(toast).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            description:
              'Budget not found: It may have been deleted by another user.',
          })
        );
      });
    });
    ...
    it('shows confirmation dialog before deleting', async () => {
      ...
      expect(window.confirm).toHaveBeenCalledWith(
        expect.stringContaining(
          'Are you sure you want to delete the budget "Operations Budget"'
        )
      );
    ...
    it('deletes budget successfully when confirmed', async () => {
      ...
      expect(toast).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          description: expect.stringContaining(
            '"Operations Budget" has been successfully deleted'
          ),
        })
      );
    });
      ...
      await waitFor(() => {
        expect(toast).toHaveBeenCalledWith(
          expect.objectContaining({
            ...
            description: expect.stringContaining(
              'Failed to delete budget "Operations Budget"'
            ),
          })
        );
      });
    ...
    it('shows loading state during initial data fetch', async () => {
      ...
      jest
        ...
        .mockImplementation(
          () =>
            new Promise((resolve) =>
              setTimeout(() => resolve({ periods: mockBudgetPeriods }), 100)
            )
        );
