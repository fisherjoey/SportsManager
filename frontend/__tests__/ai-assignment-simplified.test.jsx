...
beforeEach(() => {
  ...
  mockApi.getGames.mockResolvedValue({
    data: [
      { id: '1', homeTeam: 'Lakers', awayTeam: 'Warriors', status: 'unassigned', division: 'Premier' },
      { id: '2', homeTeam: 'Bulls', awayTeam: 'Celtics', status: 'unassigned', division: 'Premier' }
    ]
  })
  ...
  mockApi.getReferees.mockResolvedValue({
    ...
    data: { 
      referees: [
        { id: 'ref-1', name: 'John Smith', certificationLevel: 'Level 5', isAvailable: true },
        { id: 'ref-2', name: 'Sarah Johnson', certificationLevel: 'Level 4', isAvailable: true }
      ]
    }
  })
      ...
      assignments: [
        {
          gameId: '1',
          assignedReferees: [
            { refereeId: 'ref-1', refereeName: 'John Smith', confidence: 0.92, position: 'Referee 1' }
          ],
          conflicts: []
        }
      ],
...
describe('AI Assignments Enterprise - Simplified Tests', () => {
  ...
  describe('Bulk Assignment Workflow', () => {
    test('shows bulk assignment interface with scope options', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByLabelText(/All Unassigned Games/)).toBeInTheDocument()
  ...
  describe('Assignment Results Display', () => {
    test('shows assignment results after generation', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('Assignment Results')).toBeInTheDocument()
        expect(screen.getByText(/Successfully assigned 1 games/)).toBeInTheDocument()
  ...
  describe('Optimization Settings', () => {
    test('displays optimization parameters interface', async () => {
      ...
      expect(screen.getByText('Optimization Parameters')).toBeInTheDocument()
      expect(screen.getByText('Fine-tune the assignment algorithm for your organization\'s needs')).toBeInTheDocument()
