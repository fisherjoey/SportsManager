...
describe('AI Assignment Optimization Settings', () => {
  ...
  describe('Performance Metrics Calculation', () => {
    test('calculates optimization metrics correctly', async () => {
      ...
      const mockRunResponse = {
        ...
        data: {
          assignments: [
            {
              gameId: '1',
              assignedReferees: [{ confidence: 0.85 }],
              conflicts: [],
            },
            {
              gameId: '2',
              assignedReferees: [{ confidence: 0.9 }],
              conflicts: [],
            },
          ],
          conflictsFound: 0,
      ...
      await waitFor(() => {
        expect(
          screen.getByText(
            'Successfully assigned 2 games with optimization metrics'
          )
        ).toBeInTheDocument();
    ...
    test('handles assignments with conflicts', async () => {
      ...
      await waitFor(() => {
        ...
        expect(
          screen.getByText('Assignment with 1 minor conflicts')
        ).toBeInTheDocument();
      });
    });
  });
  ...
  describe('Fallback Algorithm Testing', () => {
    ...
    test('fallback algorithm respects basic optimization constraints', async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText('Unassigned Games') ||
            screen.getByText('Assignment Results')
        ).toBeInTheDocument();
      });
    });
  });
