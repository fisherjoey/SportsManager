...
describe('Auth Provider Role Functions', () => {
  describe('hasRole function', () => {
    ...
    it('should return true when user has specific role in roles array', () => {
      ...
      expect(mockAuthFunctions.hasRole(refereeUser, 'referee_coach')).toBe(
        false
      );
  ...
  describe('hasAnyRole function', () => {
    it('should return true when user has any of the specified roles', () => {
      ...
      expect(
        mockAuthFunctions.hasAnyRole(multiRoleUser, 'referee', 'admin')
      ).toBe(true);
      expect(
        mockAuthFunctions.hasAnyRole(multiRoleUser, 'admin', 'referee_coach')
      ).toBe(false);
    });
    ...
    it('should return true for admin with any role combination', () => {
      ...
      expect(
        mockAuthFunctions.hasAnyRole(adminUser, 'referee_coach', 'evaluator')
      ).toBe(true);
      expect(mockAuthFunctions.hasAnyRole(adminUser, 'nonexistent_role')).toBe(
        true
      );
    });
  ...
  describe('Role-based UI logic', () => {
    it('should correctly determine component visibility based on roles', () => {
      ...
      const refereeManagementVisible = users.map((user) => ({
        userId: user.id,
        canSeeRefereeManagement: mockAuthFunctions.hasAnyRole(
          user,
          'admin',
          'referee_coach'
        ),
    ...
    it('should handle navigation menu items based on roles', () => {
      ...
      expect(getVisibleMenuItems(adminUser)).toEqual([
        'games',
        'referees',
        'assignments',
        'reports',
      ...
      expect(getVisibleMenuItems(evaluatorUser)).toEqual([
        'games',
        'evaluations',
      ]);
      expect(getVisibleMenuItems(multiUser)).toEqual([
        'games',
        'my_referees',
        'evaluations',
      ]);
  ...
  describe('Authentication state transitions', () => {
    it('should handle role updates during session', () => {
      ...
      expect(mockAuthFunctions.hasRole(currentUser, 'referee_coach')).toBe(
        false
      );
    ...
    it('should handle logout state correctly', () => {
      ...
      expect(
        mockAuthFunctions.hasAnyRole(loggedOutUser, 'admin', 'referee')
      ).toBe(false);
    });
  });
...
describe('Auth Context Integration', () => {
  describe('Token refresh and role updates', () => {
    it('should handle role changes requiring token refresh', async () => {
      ...
      // Simulate login
      const loginResult = await mockApiClient.login(
        'test@example.com',
        'password'
      );
