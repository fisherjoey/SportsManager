...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    const initializeAuth = async () => {
      ...
      console.log('[AuthProvider] Checking stored token on mount:', !!storedToken)
      ...
      if (storedToken) {
        ...
        try {
          ...
          console.log('[AuthProvider] Profile fetched successfully:', response.user?.email)
          ...
          // Only clear if we get a 401/403, not for network errors
          if (error?.message?.includes('401') || error?.message?.includes('403') || error?.message?.includes('Unauthorized')) {
            ...
            console.warn('[AuthProvider] Backend connection failed. Keeping token for retry.')
            console.warn('[AuthProvider] Make sure backend is running on port 3001')
            // Keep the token but mark as not authenticated until we can verify
            setUser(null)
            setIsAuthenticated(false)
            // Store a flag to retry on next interaction
            sessionStorage.setItem('auth_pending_verification', 'true')
          } else {
            console.log('[AuthProvider] Unknown error, keeping token for retry:', error)
          }
        }
      } else {
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    try {
      ...
      console.log('[AuthProvider] Login response received:', !!response.token, response.user?.email)
      ...
      if (response.token && response.user) {
        ...
        // Clear any pending verification flag
