...
export function UnifiedDashboard() {
  ...
  const getPageTitle = () => {
    switch (activeView) {
      ...
      case 'admin-dashboard': return 'Administrator Dashboard'
      case 'assignor-dashboard': return 'Assignor Dashboard'
      case 'referee-dashboard': return 'Referee Dashboard'
  ...
  const getDashboardComponent = () => {
    ...
    if (userRoles.some(role =>
      ['Super Admin', 'Admin', 'admin', 'Administrator'].includes(typeof role === 'string' ? role : role.name)
    )) {
      return <AdministratorDashboard />
    }

    // Check for assignor roles
    if (userRoles.some(role =>
      ['Assignor', 'assignor', 'Assignment Manager', 'Game Assignor'].includes(typeof role === 'string' ? role : role.name)
    )) {
    ...
    if (userRoles.some(role =>
      ['Referee', 'referee', 'Senior Referee', 'Official'].includes(typeof role === 'string' ? role : role.name)
    )) {
      return <RefereeDashboardOverview />
    }

    // Default to general dashboard for other roles
    return <DashboardOverview />
  }

  // Determine if current view should be full width (for tables)
  const isFullWidthView = ['games', 'referees', 'locations', 'assignments', 'available'].includes(activeView)
  ...
  const renderContent = () => {
    ...
    switch (activeView) {
      ...
      case 'games':
        return <GamesManagementPage initialDateFilter={gameManagementDateFilter} fullWidth={true} />
      case 'assigning':
      ...
      case 'organization-compliance':
        return (
          <div className="flex flex-col items-center justify-center h-full p-8">
            <div className="text-center space-y-4 max-w-md">
              <h2 className="text-2xl font-bold">Organization Module</h2>
              <p className="text-muted-foreground">
      ...
      case 'admin-settings':
        return (
          <div className="flex flex-col items-center justify-center h-full p-8">
            <div className="text-center space-y-4 max-w-md">
              <h2 className="text-2xl font-bold">Admin Module</h2>
              <p className="text-muted-foreground">
                This module is under development.
              </p>
            </div>
  ...
  // Check if we should use SyncedSport UI (can be toggled via localStorage)
  const useSyncedSportUI = typeof window !== 'undefined' &&
    (localStorage.getItem('ui-theme') === 'syncedsport' || !localStorage.getItem('ui-theme'))
    ...
    return (
      <SidebarProvider>
        <div className="min-h-screen bg-background flex flex-col">
          ...
          {/* Main layout */}
          {/* SyncedSport Sidebar - positioned fixed */}
          <SyncedSportSidebar activeView={activeView} setActiveView={handleViewChange} />

          {/* Main content - offset for fixed sidebar */}
          <main
            className={cn(
              "mt-14 transition-all duration-200 min-h-screen",
              !isFullWidthView && "p-4"
            )}
            style={{
              marginLeft: 'var(--sidebar-current-width, 3.5rem)'
            }}
          >
            {renderContent()}
          </main>
  ...
  return (
    <SidebarProvider>
      ...
      <SidebarInset>
        ...
        <div className={cn("flex-1 overflow-auto", !isFullWidthView && "p-4")}>
          {renderContent()}
        </div>
      </SidebarInset>
    </SidebarProvider>
