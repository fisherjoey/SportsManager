...
import { UserPlus, Calendar, Clock, Zap, Users } from 'lucide-react'
    ...
    const fetchReferees = async () => {
      try {
        ...
        if (response.data) {
          ...
          referees = users.filter(user => {
            // Check if user has any referee-related roles
            const hasRefereeRole = user.roles?.some((role: { name: any; category: string }) => {
    ...
    },
    {
      ...
      accessor: (referee) => {
        ...
        if (referee.role_names && Array.isArray(referee.role_names)) {
          allRoles = referee.role_names.map((role: any) => {
            if (typeof role === 'object' && role !== null) {
              return role.name || String(role)
            }
            return String(role)
          })
        } else if (referee.roles && Array.isArray(referee.roles)) {
          // Handle both string arrays and object arrays
          allRoles = referee.roles.map((role: any) => {
            if (typeof role === 'object' && role !== null) {
              return role.name || String(role)
            }
            return String(role)
          })
        } else if (referee.roles) {
          const role: any = referee.roles
          if (typeof role === 'object' && role !== null) {
            allRoles = [role.name || String(role)]
          } else {
            allRoles = [String(role)]
          }
        } else {
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {(selectedReferee.role_names || selectedReferee.roles || ['Referee']).map((role: any, index) => (
                    <Badge key={index} variant="outline" className="text-xs">
                      {typeof role === 'object' && role !== null ? (role.name || 'Unknown Role') : String(role)}
                    </Badge>
                  ))}
                </div>
              </div>
